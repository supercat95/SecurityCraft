...
public class SCPistonTileEntity extends TileEntity implements ITickableTileEntity {
	private BlockState pistonState;
	private CompoundNBT tagIn;
	...
	public SCPistonTileEntity() {
		super(SCContent.teTypeSCPiston);
	}

	public SCPistonTileEntity(BlockState pistonStateIn, CompoundNBT tagIn, Direction pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
		this();
	...
	private BlockState getCollisionRelatedBlockState() {
		return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof ReinforcedPistonBlock ? SCContent.PISTON_HEAD.get().getDefaultState().with(PistonHeadBlock.TYPE, this.pistonState.getBlock() == SCContent.REINFORCED_STICKY_PISTON.get() ? PistonType.STICKY : PistonType.DEFAULT).with(PistonHeadBlock.FACING, this.pistonState.get(PistonBlock.FACING)) : this.pistonState;
	}
				...
				for(int i = 0; i < list1.size(); ++i) {
					...
					if (entity.getPushReaction() != PushReaction.IGNORE) {
						...
						if (!(d4 <= 0.0D)) {
							...
							entity.move(MoverType.PISTON, new Vec3d(d4 * direction.getXOffset(), d4 * direction.getYOffset(), d4 * direction.getZOffset()));
	...
	private AxisAlignedBB getMovementArea(AxisAlignedBB p_190610_1_, Direction p_190610_2_, double p_190610_3_) {
		double d0 = p_190610_3_ * p_190610_2_.getAxisDirection().getOffset();
	...
	private void fixEntityWithinPistonBase(Entity p_190605_1_, Direction p_190605_2_, double p_190605_3_) {
		...
		if (axisalignedbb.intersects(axisalignedbb1)) {
			...
			if (Math.abs(d0 - d1) < 0.01D) {
				...
				p_190605_1_.move(MoverType.PISTON, new Vec3d(d0 * direction.getXOffset(), d0 * direction.getYOffset(), d0 * direction.getZOffset()));
				MOVING_ENTITY.set((Direction)null);
			}
		}
	...
	public void clearPistonTileEntity() {
		if (this.lastProgress < 1.0F && this.world != null) {
			...
			this.remove();
			if (this.world.getBlockState(this.pos).getBlock() == SCContent.MOVING_PISTON.get()) {
				...
				//System.out.println("blockstate2: " + blockstate);
				if (tagIn != null){
					//System.out.println("pre-saving te2: " + tagIn);
					TileEntity te = blockstate.hasTileEntity() ? blockstate.createTileEntity(this.world) : null;
					if (te != null){
						//System.out.println("te2 not null:");
						te.read(tagIn);
	...
	public void tick() {
		...
		if (this.lastProgress >= 1.0F) {
			this.world.removeTileEntity(this.pos);
			this.remove();
			if (this.pistonState != null && this.world.getBlockState(this.pos).getBlock() == SCContent.MOVING_PISTON.get()) {
	...
	public void read(CompoundNBT compound) {
		...
		if (compound.contains("originalTag"))
			this.tagIn = compound.getCompound("originalTag");
	}
	...
	public CompoundNBT write(CompoundNBT compound) {
		...
		if (tagIn != null)
			compound.put("originalTag", this.tagIn);
		return compound;
	}
	...
	public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
		...
		if (this.progress < 1.0D && direction == this.getMotionDirection()) {
			...
			if (this.shouldPistonHeadBeRendered()) {
				blockstate = SCContent.PISTON_HEAD.get().getDefaultState().with(PistonHeadBlock.FACING, this.pistonFacing).with(PistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
			} else {

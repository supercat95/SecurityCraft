...
public class SCPistonBlockStructureHelper {
	...
	public SCPistonBlockStructureHelper(World worldIn, BlockPos posIn, Direction pistonFacing, boolean extending) {
		this.world = worldIn;
	...
	public boolean canMove() {
		...
		if (!ReinforcedPistonBlock.canPush(blockstate, this.pistonPos, this.world, this.blockToMove, this.moveDirection, false, this.facing)) {
	...
	private boolean addBlockLine(BlockPos origin, Direction facingIn) {
		...
		if (world.isAirBlock(origin)) {
			return true;
		} else if (!ReinforcedPistonBlock.canPush(blockstate, this.pistonPos, this.world, origin, this.moveDirection, false, facingIn)) {
			return true;
			...
			if (i + this.toMove.size() > 12) {
				...
				while(blockstate.isStickyBlock()) {
					...
					if (blockstate.isAir(this.world, blockpos) || !ReinforcedPistonBlock.canPush(blockstate, this.pistonPos, this.world, blockpos, this.moveDirection, false, this.moveDirection.getOpposite()) || blockpos.equals(this.pistonPos)) {
						break;
					}
				...
				while(true) {
					...
					if (!ReinforcedPistonBlock.canPush(blockstate, this.pistonPos, this.world, blockpos1, this.moveDirection, true, this.moveDirection) || blockpos1.equals(this.pistonPos)) {
						return false;
					}

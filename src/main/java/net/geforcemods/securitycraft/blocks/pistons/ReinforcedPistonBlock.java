...
public class ReinforcedPistonBlock extends DirectionalBlock implements IReinforcedBlock {
	...
	public ReinforcedPistonBlock(boolean sticky, Block.Properties properties) {
		super(properties);
	...
	private void checkForMove(World worldIn, BlockPos pos, BlockState state) {
		...
		if (flag && !state.get(EXTENDED)) {
			if ((new SCPistonBlockStructureHelper(worldIn, pos, direction, true)).canMove()) {
			...
			int i = 1;
			if (blockstate.getBlock() == SCContent.MOVING_PISTON.get() && blockstate.get(FACING) == direction) {
				TileEntity tileentity = worldIn.getTileEntity(blockpos);
				if (tileentity instanceof SCPistonTileEntity) {
					SCPistonTileEntity SCPistonTileEntity = (SCPistonTileEntity)tileentity;
					if (SCPistonTileEntity.isExtending() && (SCPistonTileEntity.getProgress(0.0F) < 0.5F || worldIn.getGameTime() == SCPistonTileEntity.getLastTicked() || ((ServerWorld)worldIn).isInsideTick())) {
						i = 2;
					}
				}
			}
	...
	public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
		...
		if (id == 0) {
			...
			if (tileentity1 instanceof SCPistonTileEntity) {
				((SCPistonTileEntity)tileentity1).clearPistonTileEntity();
			}

			TileEntity te = worldIn.getTileEntity(pos);
			worldIn.setBlockState(pos, SCContent.MOVING_PISTON.get().getDefaultState().with(SCMovingPistonBlock.FACING, direction).with(SCMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT), 3);
			worldIn.setTileEntity(pos, SCMovingPistonBlock.createTilePiston(this.getDefaultState().with(FACING, Direction.byIndex(param & 7)), te != null ? te.getUpdateTag() : null, direction, false, true));
			if (this.isSticky) {
				BlockPos blockpos = pos.add(direction.getXOffset() * 2, direction.getYOffset() * 2, direction.getZOffset() * 2);
				BlockState blockstate = worldIn.getBlockState(blockpos);
				Block block = blockstate.getBlock();
				boolean flag1 = false;
				if (block == SCContent.MOVING_PISTON.get()) {
					TileEntity tileentity = worldIn.getTileEntity(blockpos);
					if (tileentity instanceof SCPistonTileEntity) {
						SCPistonTileEntity SCPistonTileEntity = (SCPistonTileEntity)tileentity;
						if (SCPistonTileEntity.getFacing() == direction && SCPistonTileEntity.isExtending()) {
							SCPistonTileEntity.clearPistonTileEntity();
							flag1 = true;
						}
					}
				}
				...
				if (!flag1) {
					if (id != 1 || blockstate.isAir(worldIn, blockpos) || !canPush(blockstate, pos, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate.getPushReaction() != PushReaction.NORMAL && block != SCContent.REINFORCED_PISTON.get() && block != SCContent.REINFORCED_STICKY_PISTON.get()) {
	 ...
	 */
	public static boolean canPush(BlockState blockStateIn, BlockPos pistonPos, World worldIn, BlockPos pos, Direction facing, boolean destroyBlocks, Direction p_185646_5_) {
		Block block = blockStateIn.getBlock();
		if (block == Blocks.OBSIDIAN || block == SCContent.REINFORCED_OBSIDIAN.get()) {
			return false;
			...
			if (pos.getY() <= worldIn.getHeight() - 1 && (facing != Direction.UP || pos.getY() != worldIn.getHeight() - 1)) {
				if (block != SCContent.REINFORCED_PISTON.get() && block != SCContent.REINFORCED_STICKY_PISTON.get()) {
					...
					if (block instanceof BaseReinforcedBlock){
						if (!isSameOwner(pos, pistonPos, worldIn))
							return false;
					}
					else if (blockStateIn.getBlockHardness(worldIn, pos) == -1.0F) {
						return false;
					}
				...
				} else if (blockStateIn.get(EXTENDED) || !isSameOwner(pos, pistonPos, worldIn)) {
					return false;
				}
	...
	private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending) {
		...
		if (!extending && worldIn.getBlockState(blockpos).getBlock() == SCContent.PISTON_HEAD.get()) {
		...
		if (!SCPistonBlockStructureHelper.canMove()) {
			...
			for(int l = list.size() - 1; l >= 0; --l) {
				...
				//System.out.println("Moving block: " + blockstate3.getBlock() + " to pos " + blockpos3.offset(direction));
				TileEntity te = worldIn.getTileEntity(blockpos3);
				CompoundNBT tag = null;

				if (te != null){
					tag = new CompoundNBT();
					te.setPos(blockpos3.offset(direction));
					te.write(tag);
					//System.out.println("write tag: " + tag);
				}
				blockpos3 = blockpos3.offset(direction);
				set.remove(blockpos3);
				worldIn.setBlockState(blockpos3, SCContent.MOVING_PISTON.get().getDefaultState().with(FACING, directionIn), 68);
				worldIn.setTileEntity(blockpos3, SCMovingPistonBlock.createTilePiston(list1.get(l), tag, directionIn, extending, false));
				//System.out.println("TE: " + worldIn.getTileEntity(blockpos3).getUpdateTag());
				--k;
			...
			if (extending) {
				...
				BlockState blockstate1 = SCContent.PISTON_HEAD.get().getDefaultState().with(PistonHeadBlock.FACING, directionIn).with(PistonHeadBlock.TYPE, pistontype);
				BlockState blockstate4 = SCContent.MOVING_PISTON.get().getDefaultState().with(SCMovingPistonBlock.FACING, directionIn).with(SCMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
				set.remove(blockpos);
				TileEntity te = worldIn.getTileEntity(blockpos);
				worldIn.setBlockState(blockpos, blockstate4, 68);
				worldIn.setTileEntity(blockpos, SCMovingPistonBlock.createTilePiston(blockstate1, te != null ? te.getUpdateTag() : null, directionIn, true, true));
			}
			...
			if (extending) {
				worldIn.notifyNeighborsOfStateChange(blockpos, SCContent.PISTON_HEAD.get());
			}
	...
	private static boolean isSameOwner(BlockPos blockPos, BlockPos pistonPos, World worldIn)
	{
		if (worldIn.getTileEntity(pistonPos) instanceof OwnableTileEntity){
			return ((OwnableTileEntity)worldIn.getTileEntity(blockPos)).getOwner().getName().equals(((OwnableTileEntity)worldIn.getTileEntity(pistonPos)).getOwner().getName());
		}
		else {
			CompoundNBT originalTag = (CompoundNBT)worldIn.getTileEntity(pistonPos).getUpdateTag().get("originalTag");
			if (originalTag != null && originalTag.contains("owner")){
				return ((OwnableTileEntity)worldIn.getTileEntity(blockPos)).getOwner().getName().equals(originalTag.getString("owner"));
			}
			else
				return false;
		}
